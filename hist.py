...
def book(histos,emin=-1): ### must pass by reference!
    ...
    histos.update({"hdEdx_vs_E":   ROOT.TH2D("h_dEdx_vs_E",";E [MeV];dE/dx [MeV/#mum];Steps",bins.n_E_big,bins.Emin,bins.Emax, len(bins.dEdxbins)-1,array.array("d",bins.dEdxbins))})
    ...
    histos.update({"hdx_vs_E":     ROOT.TH2D("h_dx_vs_E",";E [MeV];dx [#mum];Steps", bins.n_E_big,bins.Emin,bins.Emax, len(bins.dxbins)-1,array.array("d",bins.dxbins))})
    histos.update({"hdxinv_vs_E":  ROOT.TH2D("h_dxinv_vs_E",";E [MeV];1/dx [1/#mum];Steps", bins.n_E_big,bins.Emin,bins.Emax, len(bins.dxinvbins)-1,array.array("d",bins.dxinvbins))})
    # histos.update({"hdL_vs_E":     ROOT.TH2D("h_dL_vs_E",";E [MeV];#DeltaL [#mum];Steps", bins.n_E_big,bins.Emin,bins.Emax, len(bins.dLbins)-1,array.array("d",bins.dLbins))})
    histos.update({"hdL_vs_E":     ROOT.TH2D("h_dL_vs_E",";E [MeV];#DeltaL [#mum];Steps", len(bins.Ebins_big)-1,array.array("d",bins.Ebins_big), len(bins.dLbins)-1,array.array("d",bins.dLbins))})
